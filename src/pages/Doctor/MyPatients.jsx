...
export default function MyPatients() {
    ...
    const fetchScanners = async () => {
        try {
            ...
            const res = await axios.get("https://medicore-connect.onrender.com/api/scanner/all-scanners", {
    ...
    const handleScanSubmit = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.post("https://medicore-connect.onrender.com/api/scanner/scan-report", {
    ...
    const fetchPatients = async () => {
        try {
            const token = localStorage.getItem("token");
            const res = await axios.get("https://medicore-connect.onrender.com/api/doctor/patients", {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchDoctors = async () => {
        try {
            const token = localStorage.getItem("token");
            const res = await axios.get("https://medicore-connect.onrender.com/api/doctor/doctors", {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleReassignSubmit = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.post("https://medicore-connect.onrender.com/api/doctor/reassign", {
                                            ...
                                            value={`https://medicore-connect-frontend.vercel.app/verify-patient?data=${btoa(JSON.stringify({
                                                ...
                                                documents: [
                                                    selectedPatientIdCard.latestBill ? {
                                                        ...
                                                        size: "150kb",
                                                        type: "pdf",
                                                        url: `https://medicore-connect.onrender.com/api/biller/view-pdf/${selectedPatientIdCard.latestBill._id}` // Optional: if we want to support direct download link usage later
