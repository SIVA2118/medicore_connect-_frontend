...
import "../../styles/Biller/BillHistory.css"; // Import shared styles
import BillModal from "../../components/BillModal";

const TodaysCollection = () => {
    const navigate = useNavigate();
    const [loading, setLoading] = useState(true);
    const [allBills, setAllBills] = useState([]); // Store all bills
    const [bills, setBills] = useState([]); // Store filtered bills
    ...
    // Apply filters whenever date or allBills changes
    useEffect(() => {
    ...
    const fetchBills = async () => {
        try {
            ...
            // applyFilter will trigger via useEffect
        } catch (error) {
    ...
    const applyFilter = () => {
        ...
        const filteredBills = allBills.filter(b => new Date(b.createdAt).toDateString() === selectedDateStr);
        ...
        <div className="biller-dashboard" style={{ height: 'auto', minHeight: '100vh' }}>
            <header className="dashboard-header" style={{ marginBottom: '1rem' }}>
                ...
                <div className="header-actions">
                    <p>{new Date(filterDate).toDateString()}</p>
                </div>
            </header>
            ...
            <div className="filter-section" style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    <label style={{ fontWeight: 500, color: '#475569' }}>Select Date</label>
                    <input
                        type="date"
                        value={filterDate}
            ...
            <section className="recent-bills-section">
                <h2>Transactions for {new Date(filterDate).toLocaleDateString()}</h2>
